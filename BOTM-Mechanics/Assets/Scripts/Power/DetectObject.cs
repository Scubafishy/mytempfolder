...
public class DetectObject : MonoBehaviour
{
    ...
    public static event Action<GameObject> HealSpawnDetected;
    ...
    public static event Action HealSpawnGone;
    ...
    private void CastSphere(String FindTag)
    {
        ...
        if (Physics.SphereCast(ray, detectRadius, out hit, detectRange, IgnoredLayerMask))
        {
            if (hit.collider.gameObject.tag == FindTag)
            {
                ...
                if (pathToObjClear)
                {
                    ...
                    else if (FindTag == "HealthSpawner")
                    {
                        Debug.Log("health spawn detected");
                        
                            OnHealSpawnDetected(hit.collider.gameObject);
        ...
        else
        {
            ...
            if (FindTag == "HealthSpawner") OnHealSpawnGone();
            if (FindTag == "Enemy")
            {
                OnEnemyObjGone();
    ...
    private void OnHealSpawnDetected(GameObject detObj)
    {
        if (HealSpawnDetected != null)
        {
            HealSpawnDetected.Invoke(detObj);
        }
    }

    private void OnHealSpawnGone()
    {
        if (HealSpawnGone != null)
        {
            HealSpawnGone.Invoke();
        }
    }
